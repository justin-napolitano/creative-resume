...
1. `future:resume-job-intake-contract`
   Define and validate normalized job posting inputs.

2. `future:resume-fit-scoring`
   Replace keyword matching with an explainable scorer that returns evidence refs and gaps.

3. `future:resume-selection-plan`
   Create deterministic resume section selection plans from fit reports.

4. `future:resume-review-packet`
   Generate review packets with risk flags, selected facts, and excluded facts.

5. `future:resume-pdf-export-worker`
   Produce tailored PDFs from approved selection plans.

6. `future:application-submission-governance`
